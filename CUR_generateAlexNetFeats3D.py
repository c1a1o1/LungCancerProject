...
def generate3DFeaturesForPatient(patID):
    patient = []
    patient.append(patID)
    featureDataAlexNet = ourAlexModel.predict_generator(dataGenerator2D(patient),val_samples=img_sli)
    numRow = featureDataAlexNet.shape[1]
    numCol = featureDataAlexNet.shape[2] * featureDataAlexNet.shape[3]
    outputFeatureData = np.zeros((img_sli, numRow, numCol))
    for ind in range(img_sli):
        for rowInd in range(numRow):
            outputFeatureData[ind, rowInd, :] = \
                np.reshape(featureDataAlexNet[ind, rowInd], numCol)
    fileNm = 'AlexNetFeatures/feats3D_conv3_'+patID+'.npy'
    np.save(fileNm,outputFeatureData)


for valInd in range(len(validationIDs)):
    print('Now processing validation volume ' + str(valInd) + ' of ' + str(len(validationIDs)))
    generate3DFeaturesForPatient(validationIDs[valInd])

for sInd in range(len(trainTestIDs)):
    print('Now processing train/test volume ' + str(sInd) + ' of ' + str(len(trainTestIDs)))
    generate3DFeaturesForPatient(trainTestIDs[sInd])
...
# fileName1 = 'transferData/alexNet3D_A_TrainTest_'+st+'.npy'
# fileName2 = 'transferData/alexNet3D_A_Validation_'+st+'.npy'
